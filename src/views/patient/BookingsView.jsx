...
export default function BookingsView() {
  ...
  const groups = useMemo(() => {
    const all = tokens || [];
    return {
      upcoming: all.filter((t) => [TOKEN_STATUS.WAITING, TOKEN_STATUS.IN_CHAMBER].includes(t.status)),
      completed: all.filter((t) => t.status === TOKEN_STATUS.COMPLETED),
      cancelled: all.filter((t) => t.status === TOKEN_STATUS.SKIPPED),
    };
  }, [tokens]);
    ...
    <div className="space-y-4">
      ...
      {rows.length === 0 ? (
        ...
        <div className="space-y-3">
          {rows.map((t) => (
            <div key={t._id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex items-center gap-4">
